...
# small aesthetic changes made by wrc 24mar2017
...
usr <- par("usr"); on.exit(par(usr))
par(usr = c(0, 1, 0, 1))
r <- abs(cor(x, y, method = "spearman", use = "na.or.complete")) #added na or complete
color <- "black"
if(!is.na(r)) {	#added colors
	if(r >= 0.25) color <- "green"
	if(r >= 0.50) color <- "orange"
	if(r >= 0.75) color <- "purple"
	if(r >= 0.90) color <- "red"
}
txt <- format(c(r, 0.123456789), digits=digits)[1]
txt <- paste(prefix, txt, sep="")
if(missing(cex.cor)) cex.cor <- 0.8/strwidth(txt)
text(0.5, 0.5, txt, cex = cex.cor, col = color) #removed text scaled
}
